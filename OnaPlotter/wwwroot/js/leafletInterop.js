...
export function addChartLayer(id, tileUrl, minZoom, maxZoom, opacity, bounds) {
    ...
    attachNativeZoomCalibrator(id, layer);
    recomputeChartOverzoom();
    restackChartOpacities();
    return true;
}

// Some charts declare maxZoom=18 in their SignalK metadata but the
// tile server only actually has tiles up to z15 or z16 -- a
// surprisingly common pattern when charts are generated by someone
// who didn't measure their own tile cache. Leaflet honours the
// declared maxNativeZoom and keeps requesting the 404-ing tiles at
// z17/z18, so the chart appears to disappear (base map shows
// through) instead of overzooming the last-good tile.
//
// Auto-calibrate: after N consecutive tileerror events at the same
// zoom, clamp maxNativeZoom to z-1. Subsequent zooms past that point
// stretch the last good tile instead of fetching ghosts. A successful
// tileload at that zoom clears the count so transient hiccups don't
// permanently downgrade a chart.
const TileErrorClampThreshold = 3;
function attachNativeZoomCalibrator(id, layer) {
    const errorsAtZ = new Map();  // z -> count
    layer.on('tileerror', (e) => {
        const z = e?.coords?.z;
        if (typeof z !== 'number') return;
        // Ignore errors in the stretch zone; those aren't a metadata
        // lie, they're Leaflet trying to fetch past the native cap
        // we've already accepted it should stretch over.
        const native = layer.options.maxNativeZoom ?? 18;
        if (z > native) return;
        const count = (errorsAtZ.get(z) || 0) + 1;
        errorsAtZ.set(z, count);
        if (count >= TileErrorClampThreshold && z > 0) {
            const newNative = z - 1;
            console.warn(
                `[overzoom] chart "${id}" claims native z${native} but z${z} 404s; capping native to z${newNative}`);
            layer.options.maxNativeZoom = newNative;
            chartNativeMax.set(id, newNative);
            errorsAtZ.clear();
            // Recompute which chart gets the top-native overzoom
            // treatment now that this one's real max shifted down.
            recomputeChartOverzoom();
            // Force Leaflet to re-pick its tile source against the new
            // maxNativeZoom so the stretched view appears immediately.
            layer.redraw();
        }
    });
    layer.on('tileload', (e) => {
        const z = e?.coords?.z;
        if (typeof z === 'number') errorsAtZ.delete(z);
    });
}
