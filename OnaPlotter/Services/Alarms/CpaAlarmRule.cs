...
/// within the configured TCPA lookahead. Buddies and moored vessels are
/// exempt; per-vessel snoozes are honoured via the evaluation context.
/// Auto-clears once no vessel matches.</summary>
public sealed class CpaAlarmRule : IAlarmRule
{
    ...
    public AlarmInfo? Check(AlarmEvaluationContext ctx)
    {
        ...
        foreach (var v in ctx.Vessels)
        {
            ...
            if (_moored.IsMoored(v, ctx.Now)) continue;       // harbour tugs, moored fishing
